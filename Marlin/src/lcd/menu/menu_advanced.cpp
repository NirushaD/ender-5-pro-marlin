    ...
    void menu_advanced_input_shaping() {
      ...
      // M593 F Frequency and D Damping ratio
        ...
        if (editable.decimal) {
          ...
          // r2: zero startup frequency must not make the Enable action a no-op.
          // This is a user-selected calibration starting point, not a tuned value.
          ACTION_ITEM_N(X_AXIS, MSG_SHAPING_ENABLE, []{ stepper.set_shaping_frequency(X_AXIS, SHAPING_FREQ_X > 0 ? SHAPING_FREQ_X : SHAPING_MIN_FREQ); });
      #endif
        ...
        if (editable.decimal) {
          ...
          ACTION_ITEM_N(Y_AXIS, MSG_SHAPING_ENABLE, []{ stepper.set_shaping_frequency(Y_AXIS, SHAPING_FREQ_Y > 0 ? SHAPING_FREQ_Y : SHAPING_MIN_FREQ); });
      #endif
